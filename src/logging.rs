...
impl LoggingSubsystem {
    ...
    pub fn module_prefix(&self) -> &'static str {
        match self {
            ...
            LoggingSubsystem::Database => "audiocontrol::helpers::settings_db,audiocontrol::helpers::attribute_cache,rusqlite",
...
mod tests {
    ...
    #[test]
    fn regression_database_subsystem_prefixes_match_current_backends() {
        let prefixes = LoggingSubsystem::Database.module_prefix();
        assert!(prefixes.contains("audiocontrol::helpers::settings_db"));
        assert!(prefixes.contains("audiocontrol::helpers::attribute_cache"));
        assert!(prefixes.contains("rusqlite"));
        assert!(!prefixes.contains("sled"));
    }

    #[test]
    fn regression_build_filter_string_expands_database_subsystem_prefixes() {
        let mut config = LoggingConfig::default();
        config.level = "warn".to_string();
        config
            .subsystems
            .insert("database".to_string(), "debug".to_string());

        let filter = config.build_filter_string();
        assert!(filter.starts_with("warn"));
        assert!(filter.contains("audiocontrol::helpers::settings_db=debug"));
        assert!(filter.contains("audiocontrol::helpers::attribute_cache=debug"));
        assert!(filter.contains("rusqlite=debug"));
    }
}
