        ...
        if let Some(api_key) = fanarttv_config.get("api_key").and_then(|v| v.as_str()) {
            let normalized_api_key = api_key.trim();
            {
                ...
                debug!("Found FanArt.tv API key in config: {}",
                       if !normalized_api_key.is_empty() && normalized_api_key.len() > 4 {
                           format!("{}...", &normalized_api_key[0..4])
                       } else {
                           "Empty".to_string()
                       });

                config.api_key = normalized_api_key.to_string();
...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    fn test_initialize_from_config_whitespace_api_key_uses_default() {
        let config = json!({
            "services": {
                "fanarttv": {
                    "enable": true,
        ...
        assert_eq!(get_api_key(), Some(default_fanarttv_api_key()));
    }

    #[test]
    #[serial_test::serial]
    fn test_initialize_from_config_trims_api_key() {
        let config = json!({
            "services": {
                "fanarttv": {
                    "enable": true,
        ...
        initialize_from_config(&config);
        assert_eq!(get_api_key(), Some("abc123".to_string()));
    }
}
