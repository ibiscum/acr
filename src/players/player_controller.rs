...
pub trait PlayerController: Send + Sync {
    ...
    ///
    /// Returns the current song metadata snapshot, or None if no song is playing.
    /// Song is optimized for now-playing state and rich metadata (album, art,
    /// source, duration, etc.).
    fn get_song(&self) -> Option<Song>;

    /// Get the queue of songs
    ///
    /// Returns a vector of queue/library-oriented tracks (can be empty if no songs are queued).
    /// Track is optimized for ordered list entries and identity (name, track/disc number,
    /// URI, optional ID), not full now-playing metadata.
