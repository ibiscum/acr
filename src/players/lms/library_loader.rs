...
impl LMSLibraryLoader {
    ...
    fn album_key(album: &serde_json::Value) -> String {
        // LMS album payloads commonly use "album" for title; keep "title" as fallback.
        let title = album["album"].as_str()
...
#[cfg(test)]
mod tests {
    use super::LMSLibraryLoader;
    use serde_json::json;

    #[test]
    fn regression_album_key_prefers_album_field_over_title() {
        let album = json!({
        ...
        let key = LMSLibraryLoader::album_key(&album);
        assert_eq!(key, "Expected Album Name|Artist|2024");
    }

    #[test]
    fn regression_album_key_falls_back_to_title_when_album_missing() {
        let album = json!({
        ...
        let key = LMSLibraryLoader::album_key(&album);
        assert_eq!(key, "Fallback Title|Artist|1999");
    }
}
