...
def test_check_favourite_status(generic_server):
    ...
    # Ensure the song is present for this test (do not depend on test execution order)
    add_response = generic_server.api_request('POST', '/api/favourites/add', json={
        "artist": test_artist,
        "title": test_title,
    })
    assert 'Ok' in add_response
    assert add_response['Ok']['success'] is True
    ...
    # Cleanup
    remove_response = generic_server.api_request('DELETE', '/api/favourites/remove', json={
        "artist": test_artist,
        "title": test_title,
    })
    assert 'Ok' in remove_response
    assert remove_response['Ok']['success'] is True
...
def test_remove_favourite_song(generic_server):
    ...
    # Ensure the song exists before removing it
    add_response = generic_server.api_request('POST', '/api/favourites/add', json=test_song)
    assert 'Ok' in add_response
    assert add_response['Ok']['success'] is True
...
def test_verify_song_removed(generic_server):
    ...
    generic_server.api_request('POST', '/api/favourites/add', json={
        "artist": test_artist,
        "title": test_title,
    })
    generic_server.api_request('DELETE', '/api/favourites/remove', json={
        "artist": test_artist,
        "title": test_title,
    })
