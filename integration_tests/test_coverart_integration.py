...
def coverart_server():
    ...
    server = AudioControlTestServer("coverart", TEST_PORTS['coverart'])
    ...
    def create_custom_config():
        ...
        # Normalize legacy keys and enforce current schema expected by runtime.
        if "settings_db" not in services and "settingsdb" in services:
            services["settings_db"] = services.pop("settingsdb")

        datastore_cfg = services.setdefault("datastore", {})
        attribute_cache_cfg = datastore_cfg.setdefault("attribute_cache", {})
        attribute_cache_cfg["dbfile"] = str(attributes_cache_dir.absolute() / "cache_attributes.db")
        datastore_cfg["image_cache_path"] = str(images_cache_dir.absolute())

        settings_db_cfg = services.setdefault("settings_db", {})
        settings_db_cfg["path"] = str((cache_dir / "settings.db").absolute())
...
class TestCoverArtAPI:
    ...
    def test_coverart_providers_available(self, coverart_server):
        ...
        assert response.status_code == 200, f"Expected 200 from methods endpoint, got {response.status_code}: {response.text}"
        data = response.json()
        assert "methods" in data, f"Methods response missing 'methods': {data}"
        assert isinstance(data["methods"], list), f"Expected 'methods' list, got: {type(data['methods'])}"

        expected_methods = {"Artist", "Song", "Album", "Url"}
        actual_methods = {m.get("method") for m in data["methods"] if isinstance(m, dict)}
        assert expected_methods.issubset(actual_methods), (
            f"Missing expected methods. expected={expected_methods}, actual={actual_methods}"
        )

        for method_entry in data["methods"]:
            assert isinstance(method_entry, dict), f"Method entry should be object: {method_entry}"
            assert "method" in method_entry, f"Method entry missing 'method': {method_entry}"
            assert "providers" in method_entry, f"Method entry missing 'providers': {method_entry}"
            assert isinstance(method_entry["providers"], list), (
                f"providers should be list for method {method_entry.get('method')}: {method_entry}"
            )
            for provider in method_entry["providers"]:
                assert "name" in provider, f"Provider missing 'name': {provider}"
                assert "display_name" in provider, f"Provider missing 'display_name': {provider}"
