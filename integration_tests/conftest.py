...
TEST_CONFIGS = {
    ...
    'cache_failure': TEST_DIR / "test_config_cache.json",
    'cache_failure_attribute': TEST_DIR / "test_config_cache.json",
}
...
class AudioControlTestServer:
    ...
    def create_config(self) -> Path:
        ...
        if "datastore" in services:
            if "attribute_cache" in services["datastore"]:
                if self.test_name == "cache_failure":
                    # Failure injection: force attribute cache DB init failure.
                    services["datastore"]["attribute_cache"]["dbfile"] = (
                        "Z:\\nonexistent-drive\\acr\\cache_attributes.db"
                        if os.name == 'nt'
                        else "/proc/1/forbidden/cache_attributes.db"
                    )
                else:
    ...
    def get_binary_path(self) -> Path:
        ...
        # Prefer debug to ensure integration tests run the latest local build.
    ...
    def start_server(self) -> bool:
        ...
        try:
            ...
            # For env-based cache failure injection, ensure the binary is rebuilt.
            if self.test_name == "cache_failure_attribute":
                project_root = Path(__file__).parent.parent
                rebuild = subprocess.run(
                    ["cargo", "build", "--bin", "audiocontrol"],
                    cwd=project_root,
                    capture_output=True,
                    text=True,
                )
                if rebuild.returncode != 0:
                    print(f"Failed to rebuild audiocontrol for {self.test_name}: {rebuild.stderr}")
                    return False
            ...
            if self.test_name == "cache_failure_attribute":
                server_env["ACR_FORCE_ATTRIBUTE_CACHE_STATS_ERROR"] = "1"
...
@pytest.fixture
def cache_failure_server():
    """Fixture for cache integration tests with injected cache backend failure"""
    server = AudioControlTestServer("cache_failure", TEST_PORTS['cache_failure'])
    assert server.start_server(), "Failed to start cache failure-injection test server"
    yield server
    server.stop_server()

@pytest.fixture
def cache_failure_attribute_server():
    """Fixture for cache tests with injected attribute stats failure"""
    server = AudioControlTestServer("cache_failure_attribute", TEST_PORTS['cache_failure_attribute'])
